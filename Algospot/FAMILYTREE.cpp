...
void travel(const vector<vector<int>> &child,int x){
	location[x]=route.size();
	route.push_back(x); 
	for(int i=0;i<child[x].size();i++){
		depth[child[x][i]]=depth[x]+1;
		travel(child,child[x][i]);
		route.push_back(x);
	}
}
...
int main(){
	...
	int TC; scanf("%d",&TC);
	while(TC--){
		route.clear(); child.clear();
		memset(location,-1,sizeof(location)); memset(depth,-1,sizeof(depth)); 
		depth[0]=0; location[0]=0;
		
		int n,q; scanf("%d %d",&n,&q);
		
		child.resize(n+50);
		for(int i=1;i<n;i++){
			int x; scanf("%d",&x); child[x].push_back(i);
		}
		travel(child,0);
		seg_make(route);
		...
		for(int i=0;i<q;i++){
			...
			printf("%d\n",depth[a]+depth[b]-depth[lca_num]*2);
		}
	}
